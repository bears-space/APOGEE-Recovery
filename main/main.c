...
static void play_note(buzzer_handle_t buzzer, servo_handle_t servo,
                      const melody_note_t* note) {
    const uint32_t duration_ms = note_duration_ms(note->length);
    ESP_ERROR_CHECK(
        servo_set_angle(servo, servo_angle_for_note(note->frequency_hz)));
...
void app_main(void) {
    ...
    while (true) {
        for (size_t i = 0; i < sizeof(melody) / sizeof(melody[0]); ++i) {
            play_note(buzzer, servo, &melody[i]);
        }
    }
}
